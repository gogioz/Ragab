    ...
    <main className="min-h-screen bg-paper">
      <div className="bg-navy py-28 px-6 text-center">
        ...
        <p className="font-body text-white/50 mt-4 text-lg max-w-xl mx-auto">
          {isAr ? "جميع تقارير أحمد رجب المنشورة" : "All published reports by Ahmed Ragab"}
        </p>
      </div>
      <div className="max-w-5xl mx-auto px-6 lg:px-10 py-20">
        {loading ? (
          ...
          <p className="font-body text-muted text-center py-20">{isAr ? "لا توجد تقارير بعد" : "No reports yet."}</p>
        ) : (
          <div className="space-y-6">
            {items.map((item) => (
              <Link key={item.id} href={`/reports/${item.id}`} dir={isAr ? "rtl" : "ltr"}
